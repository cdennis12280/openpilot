...
# do not allow to engage after a week without internet
DAYS_NO_CONNECTIVITY_MAX = 700000
# send an offroad prompt after 4 days with no internet
DAYS_NO_CONNECTIVITY_PROMPT = 400000
...
def read_thermal(thermal_config):
    ...
    dat.deviceState.memoryTempC = read_tz(
        thermal_config.mem[0]) / thermal_config.mem[1]
    dat.deviceState.ambientTempC = read_tz(
        thermal_config.ambient[0]) / thermal_config.ambient[1]
    dat.deviceState.batteryTempC = read_tz(
        thermal_config.bat[0]) / thermal_config.bat[1]
    return dat
...
def setup_eon_fan():
    ...
    try:
        ...
        # set drive current and global interrupt disable
        bus.write_byte_data(0x21, 0x03, 0x1)
...
def handle_fan_eon(max_cpu_temp, bat_temp, fan_speed, ignition):
    new_speed_h = next(speed for speed, temp_h in zip(
        _FAN_SPEEDS, _TEMP_THRS_H) if temp_h > max_cpu_temp)
    new_speed_l = next(speed for speed, temp_l in zip(
        _FAN_SPEEDS, _TEMP_THRS_L) if temp_l > max_cpu_temp)
...
def thermald_thread():
    ...
    # 2.5x the expected pandaState frequency
    pandaState_timeout = int(1000 * 2.5 * DT_TRML)
    pandaState_sock = messaging.sub_sock(
        'pandaState', timeout=pandaState_timeout)
    ...
    if EON:
        ...
        cpr_files = [
            "/sys/kernel/debug/regulator/pm8994_s11/voltage"] + cpr_files
        cpr_data = {}
        for cf in cpr_files:
            with open(cf, "r") as f:
                try:
    ...
    while 1:
        ...
        if pandaState is not None:
            ...
            set_offroad_alert_if_changed(
                "Offroad_HardwareUnsupported", not startup_conditions["hardware_supported"])
        ...
        max_comp_temp = max(
            max_cpu_temp, msg.deviceState.memoryTempC, max(msg.deviceState.gpuTempC))
        bat_temp = msg.deviceState.batteryTempC

        if handle_fan is not None:
            fan_speed = handle_fan(
                max_cpu_temp, bat_temp, fan_speed, startup_conditions["ignition"])
            msg.deviceState.fanSpeedPercentDesired = fan_speed

        # If device is offroad we want to cool down before going onroad
        # since going onroad increases load and can make temps go over 107
        # We only do this if there is a relay that prevents the car from faulting
        is_offroad_for_5_min = (started_ts is None) and ((not started_seen) or (
            off_ts is None) or (sec_since_boot() - off_ts > 60 * 5))
        if max_cpu_temp > 107. or bat_temp >= 63. or (is_offroad_for_5_min and max_cpu_temp > 70.0):
            ...
            thermal_status = clip(
                thermal_status, ThermalStatus.red, ThermalStatus.danger)
        elif max_cpu_temp > 94.0:
            # hysteresis between engage not allowed and uploader not allowed
            thermal_status = clip(
                thermal_status, ThermalStatus.yellow, ThermalStatus.red)
            ...
            thermal_status = clip(
                thermal_status, ThermalStatus.green, ThermalStatus.yellow)
        else:
        ...
        set_offroad_alert_if_changed(
            "Offroad_InvalidTime", (not startup_conditions["time_valid"]))
        ...
        update_failed_count = 0 if update_failed_count is None else int(
            update_failed_count)
        last_update_exception = params.get(
            "LastUpdateException", encoding='utf8')
        ...
        if update_failed_count > 15 and last_update_exception is not None:
            ...
            set_offroad_alert_if_changed(
                "Offroad_ConnectivityNeededPrompt", False)
            set_offroad_alert_if_changed(
                "Offroad_UpdateFailed", True, extra_text=extra_text)
            ...
            set_offroad_alert_if_changed("Offroad_ConnectivityNeeded", False)
            set_offroad_alert_if_changed(
                "Offroad_ConnectivityNeededPrompt", True, extra_text=f"{remaining_time} days.")
        else:
            set_offroad_alert_if_changed("Offroad_UpdateFailed", False)
            set_offroad_alert_if_changed("Offroad_ConnectivityNeeded", False)
            set_offroad_alert_if_changed(
        ...
        startup_conditions["up_to_date"] = params.get(
            "Offroad_ConnectivityNeeded") is None or params.get_bool("DisableUpdates")
        ...
        startup_conditions["fw_version_match"] = (panda_signature is None) or (
            panda_signature == FW_SIGNATURE)   # don't show alert is no panda is connected (None)
        set_offroad_alert_if_changed(
            "Offroad_PandaFirmwareMismatch", (not startup_conditions["fw_version_match"]))
        ...
        set_offroad_alert_if_changed(
            "Offroad_TemperatureTooHigh", (not startup_conditions["device_temp_good"]))

        if TICI:
            set_offroad_alert_if_changed(
                "Offroad_NvmeMissing", (not Path("/data/media").is_mount()))
        ...
        if manager_state is not None:
            ui_running = "ui" in (
                p.name for p in manager_state.managerState.processes if p.running)
            if ui_running_prev and not ui_running:
                HARDWARE.set_screen_brightness(20)
            ui_running_prev = ui_running

        # if current is positive, then battery is being discharged
        msg.deviceState.chargingError = current_filter.x > 0. and msg.deviceState.batteryPercent < 90
        ...
        if EON and not is_uno:
            set_offroad_alert_if_changed(
                "Offroad_ChargeDisabled", (not usb_power))
        ...
        if (count % int(600. / DT_TRML)) == 0:
            ...
            cloudlog.event("STATUS_PACKET",
                           ...
                           location=(strip_deprecated_keys(
                               location.gpsLocationExternal.to_dict()) if location else None),
