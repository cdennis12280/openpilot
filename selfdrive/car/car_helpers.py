...
def load_interfaces(brand_names):
    ...
    for brand_name in brand_names:
        ...
        CarInterface = __import__(
            path + '.interface', fromlist=['CarInterface']).CarInterface

        if os.path.exists(BASEDIR + '/' + path.replace('.', '/') + '/carstate.py'):
            CarState = __import__(path + '.carstate',
                                  fromlist=['CarState']).CarState
        else:
            CarState = None

        if os.path.exists(BASEDIR + '/' + path.replace('.', '/') + '/carcontroller.py'):
            CarController = __import__(
                path + '.carcontroller', fromlist=['CarController']).CarController
        else:
            CarController = None
    ...
    for car_folder in [x[0] for x in os.walk(BASEDIR + '/selfdrive/car')]:
        try:
            ...
            model_names = __import__(
                'selfdrive.car.%s.values' % brand_name, fromlist=['CAR']).CAR
            model_names = [getattr(
                model_names, c) for c in model_names.__dict__.keys() if not c.startswith("__")]
...
def fingerprint(logcan, sendcan):
    fixed_fingerprint = "SKODA OCTAVIA 3RD GEN"
    skip_fw_query = True
    ...
    if not fixed_fingerprint and not skip_fw_query:
        ...
        if cached_params is not None:
            ...
            if cached_params.carName == "SKODA OCTAVIA 3RD GEN":
                cached_params = None
    ...
    cloudlog.event("fingerprinted", car_fingerprint=car_fingerprint,
                   source=source, fuzzy=not exact_match)
...
def get_car(logcan, sendcan):
    ...
    if candidate is None:
        cloudlog.warning(
            "car doesn't match any fingerprints: %r", fingerprints)
        candidate = "mock"
