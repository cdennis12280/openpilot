...
IGNORE_PROCESSES = set(["rtshield", "uploader", "deleter", "loggerd", "logmessaged",
                        "tombstoned", "logcatd", "proclogd", "clocksd", "updated", "timezoned", "manage_athenad"])
...
class Controls:
    def __init__(self, sm=None, pm=None, can_sock=None):
        ...
        if self.sm is None:
            ignore = ['driverCameraState',
                      'managerState'] if SIMULATION else None
        ...
        if can_sock is None:
            can_timeout = None if os.environ.get(
                'NO_CAN_TIMEOUT', False) else 100
    ...
    def state_transition(self, CS):
        ...
        if not self.CP.enableCruise:
            self.v_cruise_kph = update_v_cruise(
                self.v_cruise_kph, CS.buttonEvents, self.enabled)
    ...
    def state_control(self, CS):
        ...
        angle_control_saturated = self.CP.steerControlType == car.CarParams.SteerControlType.angle and \
            abs(actuators.steeringAngleDeg -
    ...
    def publish_logs(self, CS, start_time, actuators, lac_log):
        ...
        speed_override = max(
            0.0, (self.LoC.v_pid + CS.cruiseState.speedOffset) * brake_discount)
        ...
        steer_angle_without_offset = math.radians(
            CS.steeringAngleDeg - params.angleOffsetAverageDeg)
        curvature = - \
            self.VM.calc_curvature(steer_angle_without_offset, CS.vEgo)
